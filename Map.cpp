...
Map::Map(sf::RenderWindow &w, const std::string &bgName, const std::vector<std::string> &objectNames) : posY(0) {
    ...
    unsigned int maxLines = (w.getSize().x + w.getSize().y)*2;
    random_device rd;
...
void Map::draw(sf::RenderWindow &w, const float camD, int posX, float speed) {
    Sprite sbg;
    sbg.setTexture(bg);
    sbg.setScale(Vector2f((float)w.getSize().x / bg.getSize().x, (float)w.getSize().y * BGS / bg.getSize().y));
    sbg.setPosition(0, 0);
    w.draw(sbg);
    ...
    {
        Line &l = lines[n%N];
        l.project(playerX*roadW-x, camH, startPos*segL - (n>=N?N*segL:0), camD, w.getSize().x, w.getSize().y, roadW);
        x += dx;
        dx += l.curve;
        ...
        drawQuad(w, grass, 0, p.Y, w.getSize().x, 0, l.Y, w.getSize().x);
